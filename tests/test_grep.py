...
def test_nonexistent_path(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        grep('pattern', 'nonexistent.txt')


def test_pattern_match(fs: FakeFilesystem) -> None:
    content = 'first line\nmatch here\nlast line'
...
def test_no_matches(fs: FakeFilesystem) -> None:
...
def test_recursive_search(fs: FakeFilesystem) -> None:
...
def test_ignore_case(fs: FakeFilesystem) -> None:
...
def test_multiple_matches(fs: FakeFilesystem) -> None:
    content = 'match first\nno match\nmatch second'
    fs.create_file('test.txt', contents=content)
...
def test_regex_pattern(fs: FakeFilesystem) -> None:
    content = '123-45-6789\nphone: 555-2288'
