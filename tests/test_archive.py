...
def test_zip_nonexistent(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        zip_folder('nonexistent_folder')


def test_zip_not_folder(fs: FakeFilesystem) -> None:
    fs.create_file('test_file.txt')
    with pytest.raises(ValueError):
        zip_folder('test_file.txt')


def test_zip_creation(fs: FakeFilesystem) -> None:
...
def test_zip_default_name(fs: FakeFilesystem) -> None:
...
def test_unzip_file(fs: FakeFilesystem) -> None:
    fs.create_dir('source')
    fs.create_file('source/file1.txt', contents='content1')
    fs.create_file('source/file2.txt', contents='content2')
...
def test_unzip_nonexistent(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        unzip_archive('nonexistent.zip')


def test_tar_creation(fs: FakeFilesystem) -> None:
    fs.create_dir('data')
...
def test_untar_file(fs: FakeFilesystem) -> None:
    fs.create_dir('source')
...
def test_untar_nonexistent(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        untar_archive('nonexistent.tar.gz')


def test_zip_with_subdirs(fs: FakeFilesystem) -> None:
...
def test_tar_with_subdirs(fs: FakeFilesystem) -> None:
