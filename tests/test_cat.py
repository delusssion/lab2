...
def test_cat_nonexistent_file(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        cat('nonexisting.txt')


def test_cat_directory_instead_file(fs: FakeFilesystem) -> None:
    fs.create_dir('test_dir')
    with pytest.raises(IsADirectoryError):
        cat('test_dir')


def test_cat_file_with_content(fs: FakeFilesystem, home_dir) -> None:
    content = 'hello\nworld'
...
def test_cat_large_file(fs: FakeFilesystem) -> None:
...
def test_cat_empty_file(fs: FakeFilesystem) -> None:
...
def test_cat_special_characters(fs: FakeFilesystem) -> None:
    content = 'тест\n中文\n🧠'
