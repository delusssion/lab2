...
def test_mv_nonexistent_source(fs: FakeFilesystem) -> None:
...
def test_mv_file_to_directory(fs: FakeFilesystem) -> None:
    content = 'test content'
...
def test_mv_directory(fs: FakeFilesystem) -> None:
...
def test_mv_overwrite_existing(fs: FakeFilesystem) -> None:
...
def test_mv_self_move_directory(fs: FakeFilesystem) -> None:
    fs.create_dir('source_dir/subdir')
    with pytest.raises(ValueError):
        mv('source_dir', 'source_dir/copy')


def test_mv_to_existing_directory(fs: FakeFilesystem) -> None:
    fs.create_file('file.txt', contents='content')
