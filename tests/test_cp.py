...
def test_cp_nonexistent_source(fs: FakeFilesystem) -> None:
    with pytest.raises(FileNotFoundError):
        cp('nonexistent.txt', 'copy.txt')


def test_cp_file(fs: FakeFilesystem) -> None:
...
def test_cp_directory_without_recursive(fs: FakeFilesystem) -> None:
    fs.create_dir('source_dir')
    with pytest.raises(NotADirectoryError):
        cp('source_dir', 'dest_dir')


def test_cp_directory_recursive(fs: FakeFilesystem) -> None:
...
def test_cp_file_to_existing_directory(fs: FakeFilesystem) -> None:
...
def test_cp_overwrite_existing_file(fs: FakeFilesystem) -> None:
...
def test_cp_self_copy_directory(fs: FakeFilesystem) -> None:
    fs.create_dir('source_dir/subdir')
    with pytest.raises(ValueError):
        cp('source_dir', 'source_dir/copy', recursive=True)


def test_cp_multiple_files_to_directory(fs: FakeFilesystem) -> None:
